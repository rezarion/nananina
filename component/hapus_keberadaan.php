<?php
	...
	$query = "DELETE FROM keberadaan_sarana WHERE id_keberadaan='$id'";
	$hasil = mysql_query($query);
	
	$status= "INSERT INTO user_log(id_log,id_user,status)
				VALUES ('','$_SESSION[id_user]','Hapus data keberadaan penduduk')";
	$stat = mysql_query($status);
	
	$_SESSION['success'] = "Data berhasil dihapus";
	echo"<meta http-equiv='refresh' content='1;url=main.php?menu=lihatkeberadaan'/>";
	...
	//Misal 1, 4, 2, 5, 3
	$query = "SELECT * FROM keberadaan_sarana ORDER BY id_keberadaan";
	$hasil = mysql_query($query);
	...
	while($data = mysql_fetch_array($hasil))
	{
		...
		$id = $data['id_keberadaan'];
		
		//proses updating ID dg nilai $no
		$query2 = "UPDATE keberadaan_sarana SET id_keberadaan = $no WHERE id_keberadaan = $id";
		mysql_query($query2);
	...
	$query = "ALTER TABLE keberadaan_sarana AUTO_INCREMENT = $no";
	mysql_query($query);
?>
