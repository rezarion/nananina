...
<?php
	...
	$id = htmlentities( htmlspecialchars (addslashes ($_GET['id_kepadatan'])));
	$query = mysql_query("SELECT * FROM kepadatan_penduduk, kelurahan, kecamatan
							WHERE id_kepadatan = '$id'
							AND kepadatan_penduduk.id_kelurahan = kelurahan.id_kelurahan
							AND kelurahan.id_kecamatan = kecamatan.id_kecamatan							
							ORDER BY tahun DESC, kecamatan.id_kecamatan ASC");
							//JOIN kepadatan_penduduk ON kepadatan_penduduk.id_kelurahan = kelurahan.id_kelurahan
							//JOIN kelurahan ON kepadatan_penduduk.id_kecamatan = kelurahan.id_kelurahan
							//JOIN kecamatan ON kelurahan.id_kecamatan = kecamatan.id_kecamatan
	//$query = mysql_query("SELECT * FROM kepadatan_penduduk WHERE id_kepadatan = '$id'");
...
<div class="widget">
	<h4 class="widgettitle">Form Ubah Data Kepadatan Penduduk</h4>
	<div class="widgetcontent">
		<form class="stdform" action="component/proses_edit_kepadatan.php?id=<?php echo $id ?>" method="post" enctype="multipart/form-data">
			
			<p>
				<label>No. </label>
				<span class="field"><input type="text" id="" name="id_kepadatan" class="input-medium" style= 'margin-left:0.1%;!important' value="<?php echo $id; ?>" readonly /></span>
			</p>
			...
			<p>
				<label>Nilai </label>
				<span class="field"> <input type="text" onkeypress="return numbersonly(event, false)" id="" name="nilai_kepadatan" class="input-xlarge" style= 'margin-left:0.1%;!important' maxlength="20" value="<?php echo $pecah['nilai_kepadatan']; ?>"  required/></span>
			</p>
