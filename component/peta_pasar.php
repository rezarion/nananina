...
function ambilpeta(){
    ...
    $.ajax({
        ...
        success: function(msg){
            ...
            for(i=0;i<msg.penempatantoko.lokasi_pasar.length;i++){
				x[i] = msg.penempatantoko.lokasi_pasar[i].x;
				y[i] = msg.penempatantoko.lokasi_pasar[i].y;
				nama_pasar[i] = msg.penempatantoko.lokasi_pasar[i].nama_pasar;
               
                //set koordinat marker
                var point = new google.maps.LatLng(parseFloat(msg.penempatantoko.lokasi_pasar[i].x),parseFloat(msg.penempatantoko.lokasi_pasar[i].y));
        		
				//masukin info marker
               	var contentString = "<table>"+
                					"<tr>"+"<td>"+String(msg.penempatantoko.lokasi_pasar[i].nama_pasar)+"</td>"+"</tr>"
                					"</table>";
               // var contentString = String(msg.penempatantoko.lokasi_pasar[i].nama_pasar);
                
                var infowindow = new google.maps.InfoWindow({
                        //content: parseFloat(msg.penempatantoko.lokasi_pasar[i].nama_pasar);
                    	//content : msg.penempatantoko.lokasi_pasar[i].nama_pasar
                    	content : contentString
                });
                
                //var infowindow = new google.maps.InfoWindow(String(msg.penempatantoko.lokasi_pasar[i].nama_pasar));
...
<body onload="peta_awal()">
	...
	<center>
		<h4><b>Peta Lokasi Pasar Tradisional Semarang</b></h4>
